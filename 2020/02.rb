...
def parse_line(line)
  regex = /^(?<min>\d*)-(?<max>\d*) (?<char>\w): (?<password>\w*)$/
  line.match(regex).named_captures.tap do |pw|
    pw['min'] = pw['min'].to_i
    pw['max'] = pw['max'].to_i
  end
end

input = DATA.each_line.map { |line| parse_line(line) }

test = %Q{
1-3 a: abcde
1-3 b: cdefg
2-9 c: ccccccccc
}.strip.each_line.map { |line| parse_line(line) }

def valid_passwords(data)
  data.select do |pw|
    character_count = pw['password'].chars.count { |char| char == pw['char'] }
    (pw['min']..pw['max']).include? character_count
  end
end

assert valid_passwords(test).size == 2
puts "Part 1: " + valid_passwords(input).size.to_s

def new_policy_valid_passwords(data)
  data.select do |pw|
    position_1 = pw['password'][pw['min'] - 1]
    position_2 = pw['password'][pw['max'] - 1]
    (position_1 == pw['char']) ^ (position_2 == pw['char'])
  end
end

assert new_policy_valid_passwords(test).size == 1
puts "Part 2: " + new_policy_valid_passwords(input).size.to_s
