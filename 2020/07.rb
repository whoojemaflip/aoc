...
class Bag
  ...
  def color
    regex = /^(?<color>.*) bags contain/
    @str.match(regex).named_captures['color']
  end

  def contents
    @contents ||= if @str.include? 'no other bags.'
      []
    else
      @str.gsub(/^.*contain /, '').split(', ').map { |bag_desc| bag_desc.match(/(?<count>\d+?) (?<color>.*) bags?/).named_captures }
    end
  end
end
...
class LuggageRules
  ...
  def rules
    @rules ||= @bags.reduce({}) { |acc, bag| acc[bag.color] = bag.contents; acc }
  end

  def inverted_rules
    @inverted_rules = rules.reduce({}) do |acc, (color, contents)|
      contents.each do |contents|
        acc[contents['color']] ||= []
        acc[contents['color']] << color
      end
      acc
    end
  end

  def subbag_count(color)
    rules[color].map do |bag_contents|
      (bag_contents['count'].to_i * subbag_count(bag_contents['color'])) + bag_contents['count'].to_i
    end.sum
  end

  def all_that_can_contain(color)
    options = inverted_rules[color]
    results = []
    while(options.size > 0)
      current = options.shift
      results << current
      if inverted_rules[current]
        options += inverted_rules[current]
      end
    end
    results.uniq
  end
...
def parse(input)
  LuggageRules.new input.strip.split("\n").map { |line| Bag.new(line) }
end
...
assert test_input.all_that_can_contain('shiny gold').size == 4
assert test_input.subbag_count('shiny gold') == 32

puts "Part 1: " + input.all_that_can_contain('shiny gold').size.to_s
puts "Part 2: " + input.subbag_count('shiny gold').to_s
