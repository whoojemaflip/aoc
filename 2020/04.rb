...
class Passport
  ...
  def_delegators :@obj, *REQUIRED_KEYS

  def initialize(line)
    pairs = line.split(/\s+/).map { |kv| kv.split(':') }
    @fields = pairs.reduce({}) do |acc, (k,v)|
      acc.tap { |ac| ac[k.to_sym] = v }.sort.to_h
    end
    @obj = OpenStruct.new(@fields)
  end

  def valid_fields?
    @fields.keys - OPTIONAL_KEYS == REQUIRED_KEYS
  end

  def valid_values?
    return false unless valid_fields?
...
assert test.count { |p| p.valid_fields? } == 2

invalid_test_passports = %Q{
eyr:1972 cid:100
...
assert invalid_test_passports.all? { |p| !p.valid_values? }

valid_test_passports = %Q{
...
assert valid_test_passports.all? { |p| p.valid_values? }

puts "Part 1: " + input.count { |p| p.valid_fields? }.to_s
puts "Part 2: " + input.count { |p| p.valid_values? }.to_s
