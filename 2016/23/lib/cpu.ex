...
defmodule Cpu do
  ...
  def main(_), do: run("data.txt")
  def run(filename), do: exec(%Registers{a: 12}, load(filename))

  @doc """
    iex> Cpu.test_run
    %Registers{pc: 7, a: 42, b: 0, c: 0, d: 0}
  """
  def test_run, do: exec(%Registers{}, load("simple_test.txt"))

  @doc """
    iex> Cpu.test2
    %Registers{a: 3, b: 0, c: 0, d: 0, pc: 8}
  """
  def test2, do: exec(%Registers{}, load("test2.txt"))

  def exec(registers, memory), do: _exec({registers, memory}, Enum.count(memory), 0, time)

  def time, do: :os.system_time(:millisecond)
  def milliseconds_elapsed(start_time), do: time - start_time
  def cycles_per_second(elapsed_time, number_of_cycles) do
    Float.floor (elapsed_time / 1000) * number_of_cycles
  end

  def _exec({registers, memory}, highmem, cycles, in_time) do
    break = 1000000
    out_time = case rem(cycles, break) do
      1 ->
        str = milliseconds_elapsed(in_time) |> cycles_per_second(break)
        IO.puts "#{str} cycles/s"
    ...
    if registers.pc >= highmem do
      ...
      command_at(memory, registers.pc)
        |> exec_cmd({registers, memory})
        |> increment_program_counter
        |> _exec(highmem, cycles + 1, out_time)
    end
  end

  def command_at(memory, address), do: Enum.at(memory, address)

  def increment_program_counter({%Registers{pc: pc, a: a, b: b, c: c, d: d}, memory}) do
    {%Registers{pc: pc + 1, a: a, b: b, c: c, d: d}, memory}
  end
  ...
  def exec_cmd(%{"ins" => "jnz", "arg1" => arg1, "arg2" => arg2}, {registers, memory}) do
    comp = registers |> resolve_arg(arg1)
    val = registers |> resolve_arg(arg2)

    if comp != 0 do
      {set(registers, registers.pc + val - 1, "pc"), memory}
    else
      {registers, memory}
    end
  end

  def exec_cmd(%{"ins" => "cpy", "arg1" => arg1, "arg2" => arg2}, {registers, memory}) do
    val = registers |> resolve_arg(arg1)
    {set(registers, val, arg2), memory}
  end

  def exec_cmd(%{"ins" => "inc", "arg1" => reg}, {registers, memory}) do
    val = registers |> resolve_arg(reg)
    {set(registers, val + 1, reg), memory}
  end

  def exec_cmd(%{"ins" => "dec", "arg1" => reg}, {registers, memory}) do
    val = registers |> resolve_arg(reg)
    {set(registers, val - 1, reg), memory}
  end
