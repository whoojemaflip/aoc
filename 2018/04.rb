line_regex = %r{^\[(?<date>\d{4}-\d{2}-\d{2}) (?<hour>\d{2}):(?<minute>\d{2})\] (?<remainder>.*)$}
guard_regex = /Guard #(?<guard>\d*) begins shift/

input = DATA.each_line.sort.map do |line|
  vals = line.match(line_regex).named_captures
  minute = vals['minute'].to_i
  if guard_regex.match?(line)
    {
      guard: line.match(guard_regex).named_captures['guard'].to_i,
    }
...
input.each do |ln|
    if ln[:guard]
      current_guard = ln[:guard]
    elsif ln[:sleeps]
      fell_asleep_at = ln[:sleeps]
    elsif ln[:wakes]
      guards[current_guard] ||= []
      guards[current_guard].concat (fell_asleep_at .. ln[:wakes] - 1).to_a
    end
end

guard_id = guards
  .reduce({}) { |acc, (gid, mins)| acc[gid] = mins.size; acc }
  .max_by { |gid, minutes_asleep| minutes_asleep }
  .first

most_commonly_slept_through_minute = guards[guard_id]
  .group_by(&:itself)
  .reduce({}) { |acc, (min, mins)| acc[min] = mins.size; acc }
  .max_by { |min, size| size }
  .first

puts guard_id * most_commonly_slept_through_minute

guard_id, (minute, count) = guards.reduce({}) do |acc, (gid, mins)|
    max_minute = mins.inject(Hash.new(0)) { |h, v| h[v] += 1; h }.max_by { |k,v| v }
    acc[gid] = max_minute
    acc
end.max_by { |gid, (min, cnt)| cnt }
